...
一条命令。它会自动识别你的 coding agent（Claude Code、Codex、OpenCode、Cursor……
共 70+ 种），把整份 skill——脚本**和**网页 UI——一并装进对应的 skills 目录：
...
| | |
|---|---|
| 所有项目（用户级） | `npx skills add NeoFantom/the-snap -g` |
| 指定某个 agent | `npx skills add NeoFantom/the-snap -a claude-code` |
| 只预览、不安装 | `npx skills add NeoFantom/the-snap --list` |
| 之后更新 | `npx skills update the-snap` |

底层是 [`skills`](https://github.com/vercel-labs/skills) CLI——无需全局安装，
`npx` 按需拉取。

<details><summary>手动安装（不用 npx）</summary>

这个仓库本身**就是** skill（SKILL.md 在根目录，`scripts/` 和 `web/` 与它同级），
所以把 agent 的 skills 目录指向一份 clone 即可：

```bash
git clone https://github.com/NeoFantom/the-snap
ln -s "$(pwd)/the-snap" ~/.claude/skills/the-snap   # Claude Code
# 或 ~/.codex/skills/the-snap · ~/.config/opencode/skill/the-snap · …
```

</details>

装好后，用自然语言描述任务即可——*「audit this machine before I wipe it」*、
*「迁移」*、*「响指」*——agent 就会加载 skill 并照流程走。触发短语写在 `SKILL.md`
的 `description` / `when_to_use` 里。
